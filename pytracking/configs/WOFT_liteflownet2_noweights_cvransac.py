...
from pytracking.utils.least_squares_H import find_homography_cvransac, torch_proj_errors
...
def find_homography(pts_A, pts_B, weights=None):
    ...
    res = find_homography_cvransac(pts_A, pts_B, weights=weights, max_iters=10000, thr=3)
    return res
...
def subsampler(coords_a, coords_b, weights):
    ...
    return coords_a[:, subsampling_mask], coords_b[:, subsampling_mask], None
...
def get_config():
    ...
    conf.flow_config = load_config(repo_root / 'pytracking/optical_flow/configs/mmflow-liteflownet2.py')
