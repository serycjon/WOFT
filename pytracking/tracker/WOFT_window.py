...
from ltr.data import geom_utils as gu
...
class WOFTWindow():
    ...
    def init(self, img, mask, img_identifier=None):
        ...
        if self.C.search_window_margin:
            template_bbox = gu.Bbox.from_mask(mask > 0)
            self.search_bbox = template_bbox.with_margins(self.C.search_window_margin)
            self.search_bbox = self.search_bbox.intersection(gu.Bbox(0, 0, img.shape[1], img.shape[0]))
            self.min_window_sz = 8 * 20
            self.search_bbox = self.search_bbox.with_margins_min_size(self.min_window_sz)
        else:
            self.search_bbox = gu.Bbox(0, 0, img.shape[1], img.shape[0])


        # self.search_crop_H = np.array([[1, 0, -self.search_bbox.tl_x],
        #                                [0, 1, -self.search_bbox.tl_y],
        #                                [0, 0, 1.0]])
        # self.undo_search_crop_H = np.array([[1, 0, self.search_bbox.tl_x],
        #                                     [0, 1, self.search_bbox.tl_y],
        #                                     [0, 0, 1.0]])
    ...
    def track(self, input_img, debug=False, img_identifier=None):
        ...
        pw_mask = cv2.warpPerspective(np.ones(input_img.shape[:2]), prewarp_H,
                                      (input_img.shape[1], input_img.shape[0]),
                                      flags=cv2.INTER_LINEAR)
        pw_mask = self.search_bbox.crop_image(pw_mask)
        pw_mask = torch.from_numpy(pw_mask > 0).to(self.device)
        logger.debug(f"prewarp_timer(): {prewarp_timer()}ms")

        template_img = self.search_bbox.crop_image(self.template_img)
        prewarped_input = self.search_bbox.crop_image(prewarped_input)

        if debug:
            cv2.imshow("cv: template_img", template_img)
            cv2.imshow("cv: prewarped_input", prewarped_input)
        flow_input_shape = prewarped_input.shape
        logger.debug(f"template_img.shape: {template_img.shape}")

        global_TC_timer = time_measurer('ms')
        # global_flow_timer = cuda_time_measurer('ms')
        global_flow_timer = time_measurer('ms')
        template_coords, cur_pw_coords, weights = self.flower.compute_flow(template_img, prewarped_input, mode='TC', vis=False,
                                                                           do_sigmoid=True)
        extra_vis = False
        if extra_vis:
            ...
            np_weights = (to_numpy(extra_vis_weights) * 255).astype(np.uint8).reshape(flow_input_shape[0], flow_input_shape[1])
            import os
            import sys
        ...
        H_global_cur2init = compose_H(prewarp_H, H_undo_crop(self.search_bbox, np_H_prewarped2init))
        ...
        if global_H_success:
            ...
            if self.C.no_local_H:
                ...
                if self.C.search_window_margin:
                    local_search_bbox = gu.Bbox.from_mask(prev_mask)
                    local_search_bbox = local_search_bbox.with_margins(self.C.search_window_margin)
                    local_search_bbox = local_search_bbox.intersection(gu.Bbox(0, 0, input_img.shape[1], input_img.shape[0]))
                    local_search_bbox = local_search_bbox.with_margins_min_size(self.min_window_sz)
                else:
                    local_search_bbox = gu.Bbox(0, 0, input_img.shape[1], input_img.shape[0])


                prev_img_window = local_search_bbox.crop_image(self.prev_img)
                input_img_window = local_search_bbox.crop_image(input_img)

                prev_coords, cur_coords, weights = self.flower.compute_flow(prev_img_window, input_img_window, mode='TC',
                ...
                try:
                    ...
                    H_flow = H_undo_crop(local_search_bbox, H_flow)
        ...
        if debug:
            ...
            pw_flow, pw_weights = self.flower.compute_flow(template_img, prewarped_input, mode='flow', numpy_out=True, do_sigmoid=True)
            ...
            vis_pw_flow = vis_utils.vis_flow_align(pw_flow, template_img, prewarped_input, grid_sz=5,
            ...
            local_flow, _ = self.flower.compute_flow(self.prev_img, input_img, mode='flow',
                                                     src_img_identifier=None, numpy_out=True)
            ...
            prewarp = vis_utils.vis_alignment_plain(template_img, prewarped_input)
    ...
    def _mask_coords(self, template_coords, cur_coords, weights, post_weights, pw_mask=None,
                     do_pw_mask=True, crop_bbox=None):
        if crop_bbox is None:
            template_mask = self.template_mask
        else:
            template_mask = self.template_mask[crop_bbox.tl_y:crop_bbox.br_y,
                                               crop_bbox.tl_x:crop_bbox.br_x]

        in_template_mask = template_mask[template_coords[1, :], template_coords[0, :]]
    ...
    def _mask_coords_flow(self, prev_coords, cur_coords, weights, post_weights, search_window):
        prev_mask = cv2.warpPerspective(self.np_template_mask, np.linalg.inv(self.prev_H2init),
                                        (self.template_mask.shape[1], self.template_mask.shape[0]),
                                        flags=cv2.INTER_NEAREST) > 0
        prev_mask = search_window.crop_image(prev_mask)
...
def H_undo_crop(crop_bbox, H_from_cropped_to_cropped):
    H_crop = np.array([[1, 0, -crop_bbox.tl_x],
                       [0, 1, -crop_bbox.tl_y],
                       [0, 0, 1.0]])
    H_uncrop = np.array([[1, 0, crop_bbox.tl_x],
                         [0, 1, crop_bbox.tl_y],
                         [0, 0, 1.0]])
    return compose_H(H_crop, H_from_cropped_to_cropped, H_uncrop)
